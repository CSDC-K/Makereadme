...
pub async fn create_communication(
    ...
    printd!(
        format!("Sending request to GROQ (model: {})", normalized_model).as_str(),
        Debug
    );

    let client = Client::new();
    let mut text = send_groq_request(&client, &url, &normalized_api_key, &request_body).await?;

    loop {
        sleep(Duration::from_secs(3)).await;

        println!("\n\x1b[36m================== GROQ RAW RESPONSE ==================\x1b[0m");
        println!("{}", text);
        println!("\x1b[36m=======================================================\x1b[0m\n");

        let mut action_results: Vec<ActionResult> = Vec::new();

        printd!("Parsing actions from GROQ response...", Action);
        ...
        if should_exit {
            printd!("EXIT action received. Stopping GROQ loop.", Success);
            return Ok(true);
        }

        text = create_groq_response(
...
async fn create_groq_response(
    ...
    send_groq_request(&client, &url, &api_key, &request_body).await
}
...
async fn send_groq_request(
    ...
    let response = client
        ...
        .send()
        .await
        .map_err(|e| Error::RunError(format!("GROQ request failed: {}", e)))?;
    ...
    if !status.is_success() {
        ...
        return Err(Error::RunError(format!("GROQ HTTP {}: {}", status, error_text)));
    }

    let body: Value = response
        .json()
        .await
        .map_err(|e| Error::RunError(format!("GROQ parse error: {}", e)))?;
    ...
    if let Some(message) = body
        ...
        .and_then(|m| m.as_str())
    {
        return Err(Error::RunError(format!("GROQ error: {}", message)));
    }
